...
let ResearcherService = class ResearcherService {
    ...
    async previewPublicationFromDoi(doiInput) {
        try {
            ...
            if (err.name === 'AbortError' || err.message === 'unreachable') {
                throw new common_1.BadRequestException('Crossref is not responding right now — please try again in a moment.');
            }
            throw err;
        }
    }
    async createPublicationFromDoi(userId, doiInput, file) {
    ...
    async fetchOrcidWorksList(orcidInput) {
        ...
        if (!res.ok) {
            throw new common_1.BadRequestException('Could not find an ORCID record for that ID');
        }
        const json = await res.json();
    ...
    async fetchOrcidWorkDetail(orcid, putCode) {
    ...
    async createPublicationsFromOrcid(userId, orcidInput, putCodes) {
        const orcid = orcidInput.trim().replace(/^https?:\/\/(www\.)?orcid\.org\//i, '');
        const list = await this.fetchOrcidWorksList(orcid);
        const selected = list.filter((w) => putCodes.includes(w.putCode));
        const saved = [];
        for (const item of selected) {
            ...
            if (item.doi) {
                try {
                    pubData = await this.fetchCrossrefMetadata(item.doi);
                }
                catch {
                    pubData = null;
                }
    ...
    async updateProfile(userId, body, file) {
        ...
        if (body.graduation_country !== undefined)
            updateData.graduation_country = body.graduation_country;
