...
let ProfileService = class ProfileService {
    ...
    async completeResearcherProfile(targetId, dto) {
        const user = await this.findOrFail(targetId);
        if (user.user_category !== user_entity_1.UserCategory.RESEARCHER) {
            throw new common_1.ForbiddenException('Only researcher accounts have this profile step');
        }
        const tokenValid = user.emailVerificationToken &&
            user.emailVerificationToken === dto.token &&
            user.emailVerificationExpires &&
            user.emailVerificationExpires > new Date();
        if (!tokenValid) {
            throw new common_1.UnauthorizedException('This link has expired. Please sign up again or contact support.');
        }
        const fields = [
